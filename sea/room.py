    ...
    def map(self, freqs=[], opacity=0.5):
        ...
        for fi, f in enumerate(freqs):
            ...
            for i, node in enumerate(x):
                fig.add_trace(go.Scatter3d(x=[x[i]], y=[y[i]], z=[z[i]], marker=dict(size=10, cmax=max_val, cmid=mid_val, cmin=min_val, colorscale='edge', symbol='circle'), showlegend=False))
